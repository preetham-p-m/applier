...
def check_int(
    ...
    if var < min_value:
        raise ValueError(
            f'The variable "{var_name}" in "{__validation_file_path}" expects an Integer greater than or equal to `{min_value}`! Received `{var}` instead!\n\nSolution:\nPlease open "{__validation_file_path}" and update "{var_name}" accordingly.'
        )
    return True
...
def check_list(
    ...
    if len(var) < min_length:
        raise ValueError(
            f"Invalid input for {var_name}. Expecting a List of length at least {min_length}!"
        )
    for element in var:
        if not isinstance(element, str):
            raise TypeError(
                f"Invalid input for {var_name}. All elements in the list must be strings!"
            )
        if len(options) > 0 and element not in options:
            raise ValueError(
                f'Invalid input for {var_name}. Expecting all elements to be values from {options}. This "{element}" is NOT in options!'
            )
    return True
